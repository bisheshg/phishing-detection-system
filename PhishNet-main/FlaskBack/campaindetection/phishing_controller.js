...
import { analyzeBehavior } from '../utils/behavioralAnalyzer.js';
...
export const analyzeUrl = async (req, res, next) => {
  ...
  try {
    ...
    try {
      ...
      // 🛡️ BEHAVIORAL INTELLIGENCE LAYER (Phase E) ──────────────────────────
      const behavioralData = await analyzeBehavior(userId, fingerprint, req.ip);
      
      // Critical Audit: Log high-risk behavioral anomalies
      if (behavioralData.threatActorLikelihood > 70) {
         await AuditLog.create({
           action: "HIGH_RISK_SCANNER_DETECTED",
           userId,
           fingerprint,
           severity: "Critical",
           details: { url: url.trim(), behavioralData }
         });
      }
      
      // Attach to mlResult temporarily for the ScanHistory constructor
      mlResult.behavioralData = behavioralData;
    } catch (flaskError) {
    ...
    const scanHistory = new ScanHistory({
      ...
      behavioralContext: mlResult.behavioralData,
    });

    await scanHistory.save();

    // ── ANTI-POISONING & OUTLIER DETECTION (Phase D) ──────────────────────
    let isOutlier = false;
    const robustness = mlResult.adversarial_robustness || { stability_score: 1.0 };
    
    if (robustness.stability_score < 0.4) {
      isOutlier = true;
      console.warn(`[SECURITY] Outlier detected: ${mlResult.url} (Stability: ${robustness.stability_score}) — Isolating from training pipeline.`);
      scanHistory.isOutlier = true;
      await scanHistory.save();
    }

    // ── CAMPAIGN CORRELATION (Hardened Production logic) ───────────────────
    try {
      const { campaign_signature } = mlResult;
      const Campaign = (await import('../models/Campaign.js')).default;
      
      let campaign = null;
      if (mlResult.prediction === 'Phishing' && campaign_signature) {
          // Cluster search: find campaign by HTML hash, Server IP, or Semantic Similarity
          campaign = await Campaign.findOne({
            $or: [
              { 'signatures.html_hash': campaign_signature.html_hash },
              { 'signatures.server_ip': campaign_signature.server_ip },
              // Note: In production, use a vector DB for semantic similarity ($near)
              // Here we store it for graph-based correlation in the dashboard
            ],
            status: 'Active'
          });
          
          // Enhanced signature update
          if (campaign && campaign_signature.semantic_embedding) {
             campaign.signatures.semantic_embedding = campaign_signature.semantic_embedding;
          }
          ...
          if (campaign) {
            ...
            // Prevent duplicate URLs in detections list (simple version)
            if (!campaign.detectedUrls.some(d => d.url === mlResult.url)) {
                campaign.detectedUrls.push({ url: mlResult.url });
            }
            await campaign.save();
          } else if (campaign_signature.html_hash || campaign_signature.server_ip) {
            // Create new campaign if signatures are significant
            campaign = new Campaign({
              signatures: {
                html_hash: campaign_signature.html_hash,
                server_ip: campaign_signature.server_ip,
                semantic_embedding: campaign_signature.semantic_embedding
              },
              detectedUrls: [{ url: mlResult.url }]
            });
            await campaign.save();
          }
      }
      ...
      console.error('[CAMPAIGN] Correlation error:', campaignErr.message);
    }
    // ─────────────────────────────────────────────────────────────────────
...
export const getCampaigns = async (req, res, next) => {
  try {
    const Campaign = (await import('../models/Campaign.js')).default;
    // Get top 20 active campaigns sorted by last seen
